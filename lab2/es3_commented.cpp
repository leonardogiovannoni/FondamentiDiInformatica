...
Obiettivo
--------------------------------------------------------------------------------
Dati due numeri naturali a e b, calcolare il loro
massimo comune divisore: MCD(a, b).

Nota sul lessico (solo in QUESTO commento):
- Usiamo "//" per indicare la divisione intera,
  per evitare confusione con la divisione reale (non c'è ambiguità perché siamo
  in ℕ, ma è più chiaro).
...
Iterando:
    MCD(a, b) = MCD(b, a % b) = MCD(a % b, b % (a % b)) = ... = ultimo secondo elemento != 0
...
Dobbiamo dimostrare che MCD(a, b) = MCD(b, r) con r = a % b.
Per fare ciò, basta dimostrare che `a` e `b` hanno gli stessi divisori comuni di `b` e `r`.
...
da cui segue che MCD(a, b) = MCD(b, r)

Basta provare le due inclusioni:

---------------------------------------------------------------------
(2)  Se d ∈ D(a, b)  allora d ∈ D(b, r)      [quindi D(a,b) ⊆ D(b,r)]                   (Se `d`
...
---------------------------------------------------------------------
(3)  Se d ∈ D(b, r)  allora d ∈ D(a, b)      [quindi D(b,r) ⊆ D(a,b)]                   (Se `d`
