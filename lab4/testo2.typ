#set page(
  width: 21cm, 
  height: 29.7cm,
  margin: (top: 2.54cm, bottom: 2.54cm, left: 2.54cm, right: 2.54cm)
)
#set text(size: 12pt, font: "New Computer Modern")
#set par(leading: 0.65em, spacing: 0.65em)
#set block(spacing: 0.65em)

// Configuration for bullet lists
#show list: set block(above: 1.2em, below: 1.2em)
#set list(spacing: 1.2em)

// Configuration for numbered lists
#show enum: set block(above: 1.2em, below: 1.2em)
#set enum(spacing: 1.2em)

#show title: set block(above: 1.4em, below: 1em)
#show title: it => [
  #v(1em)
  #align(center)[
    #set text(size: 23pt, weight: "regular")
    #it.body
  ]
]
#title[Esercizio: Verifica ordine crescente in un array]

#v(7em)

Scrivi un programma in linguaggio *C++* che:

- dichiari un array di dimensione fissa $N = 6$;
- legga da tastiera $N$ numeri interi e li memorizzi nell'array;
- stampi a video l'intero contenuto dell'array;
- verifichi se gli elementi dell'array sono disposti in ordine *strettamente crescente*;
- stampi a video `L'array e' crescente` se l'array è crescente, altrimenti `L'array non e' crescente`.

#v(10pt)
== Esempi

=== Input:
```text
1 3 5 7 9 12
```

=== Output:

```text
Array: 1 3 5 7 9 12 
L'array e' crescente
```

#v(1em)

=== Input:

```text
2 4 8 6 10 12
```

=== Output:

```text
Array: 2 4 8 6 10 12 
L'array non e' crescente
```
